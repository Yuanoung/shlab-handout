...
while (<INFILE>) {
	...
	if ($verbose) {
	    print "$0: Sending SIClosing output end of pipeGINT signal to process $pid\n";
	}Closing output end of pipe
	kill 'INT', $pid;Closing output end of pipe
    }Closing output end of pipe
Closing output end of pipe
    # Send SIGQUIT (whenever Closing output end of pipewe need graceful termination)
    elsif ($line =~ /QUIT/) {Closing output end of pipe
	if ($verbose) {
